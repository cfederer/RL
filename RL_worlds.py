...
class callies_world():
    ...
    """
    def __init__(self):
        print("It's Callie's World...and you're all just living in it")
        self.name = "callies_world"
        self.n_states = 100
        self.n_actions = 4
        self.dim_x = 10
        self.dim_y = 10
        self.C_states = [83, 84, 84, 73, 63, 53, 43, 33, 23, 13, 14, 15]
        self.C_reward = -1 
        self.bigR_state = 00
        self.lilR_state = 99
        self.bigR_reward = 2
    ...
    def get_outcome(self, state, action):
        if (state==99 or state==0): #goal states
            reward = 0
            next_state = None
        ...
        if(action==0): # move right
            next_state = state + 1
            if(state%10 ==9): ## hit right border
                next_state = state
            if(state in self.C_states):
                reward = self.C_reward
            elif(state ==self.bigR_state):
                reward = self.bigR_reward
                next_state = None
            elif(state==self.lilR_state):
                reward = self.lilR_reward
                next_state = None 
        elif(action==1): #move up 
            next_state = state + 10
            if(state >= 90): ## hit top border
                next_state = state
            if(state in self.C_states):
                reward = self.C_reward
            elif(state ==self.bigR_state):
